...
public sealed class ExchangeEndPointCmdHandler : CommandHandlerBase<ExchangeEndPointCmd, ExchangeEndPointCmd.Response>
{
    private readonly ReceivedMessages _receivedMessages;

    public ExchangeEndPointCmdHandler(ReceivedMessages receivedMessages)
    {
        _receivedMessages = receivedMessages;
    }

    public override async Task<IHandlerResult> HandleAsync(
        ExchangeEndPointCmd cmd,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (cmd.ReturnError)
        {
            return Error(new ExchangeEndPointCmd.Response { Id = cmd.Id }, $"Error for {cmd.Id}");
        }
        ...
        {
            return Error(999, new ExchangeEndPointCmd.Response { Id = cmd.Id });
        }

        return cmd.BadRequest
            ? BadRequest(new ExchangeEndPointCmd.Response { Id = cmd.Id }, "Validation error")
            : Ok(new ExchangeEndPointCmd.Response { Id = cmd.Id });
    }
}